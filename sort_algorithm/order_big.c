...
/*   Updated: 2021/10/06 13:27:31 by ecruz-go         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push_quarters(t_stack **stacka, t_stack **stackb, int turn, int size)
{
	...
	while (flag != 1)
	{
		stackaux == stack_a_end ? flag = 1 : 0;
		if (turn == 1 && stackaux->index <= size / 4)
			push_a_quarter(stacka, stackb, (size / 4) / 2);
		else if (turn == 2 && stackaux->index > size / 4 && stackaux->index <= size / 2)
			push_a_quarter(stacka, stackb, ((size / 2) / 4) * 3);
		else if (turn == 3 && stackaux->index > size / 2 && stackaux->index <= (size / 4) * 3)
